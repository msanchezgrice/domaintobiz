-- Fixed Schema Update - Handles JSONB casting properly
...
-- Insert some demo data for testing (with proper JSONB casting)
INSERT INTO generated_websites (
  ...
  'demo_' || floor(extract(epoch from now())),
  'completed',
  NOW()
), (
  'testbiz.com',
  'testbiz.com', 
  ('{"strategy": {"businessModel": {"type": "E-commerce"}, "brandStrategy": {"positioning": "Premium Online Store"}}, "executionId": "test_' || floor(extract(epoch from now())) || '"}')::jsonb,
  'https://domaintobiz.vercel.app/sites/testbiz-com-' || floor(extract(epoch from now())) || '/',
  ...
  'test_' || floor(extract(epoch from now())),
  'completed',
  NOW()
), (
  'aimarketing.com',
  'aimarketing.com',
  '{"strategy": {"businessModel": {"type": "SaaS"}, "brandStrategy": {"positioning": "AI Marketing Platform"}}, "executionId": "ai_marketing_demo"}'::jsonb,
  'https://domaintobiz.vercel.app/sites/aimarketing-com-demo/',
  '<!DOCTYPE html><html><head><title>AI Marketing Platform</title><style>body{font-family:Inter,sans-serif;margin:0;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;min-height:100vh}.container{max-width:1200px;margin:0 auto;padding:60px 20px;text-align:center}h1{font-size:3rem;margin-bottom:1rem}p{font-size:1.2rem;margin-bottom:2rem;opacity:0.9}.cta{background:white;color:#667eea;padding:15px 30px;border-radius:8px;text-decoration:none;font-weight:bold;display:inline-block}</style></head><body><div class="container"><h1>AI Marketing Platform</h1><p>Transform your marketing with artificial intelligence</p><a href="#" class="cta">Start Free Trial</a></div></body></html>',
  'body{font-family:Inter,sans-serif;margin:0;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;min-height:100vh}.container{max-width:1200px;margin:0 auto;padding:60px 20px;text-align:center}h1{font-size:3rem;margin-bottom:1rem}p{font-size:1.2rem;margin-bottom:2rem;opacity:0.9}.cta{background:white;color:#667eea;padding:15px 30px;border-radius:8px;text-decoration:none;font-weight:bold;display:inline-block}',
  'console.log("AI Marketing platform loaded"); document.querySelector(".cta").addEventListener("click", () => alert("Welcome to AI Marketing!"));',
  'ai_marketing_demo',
  'completed',
  NOW()
